...
def simulation(N,
      ...
      # Columns: log_moneyness, r, q, bs_vol, credit_spread, coupon_rate, maturity_years, conversion_ratio
      sampler = qmc.LatinHypercube(d=8)
      lhs_unit = sampler.random(n=N)

      lower = np.array([-5.0, 0.00, 0.00, 0.05, 0.001, 0.00, 0.25, 0.5])
      upper = np.array([ 4.0, 0.10, 0.15, 0.80, 0.30,  0.10, 30.0, 200.0])
      lhs_scaled = qmc.scale(lhs_unit, lower, upper)

      # Derive spot from log_moneyness and conversion_ratio
      redemption = 100.0
      log_moneyness = lhs_scaled[:, 0]
      conversion_ratio_all = lhs_scaled[:, 7]
      conversion_price_all = redemption / conversion_ratio_all
      S_all = np.clip(conversion_price_all * np.exp(log_moneyness), 0.01, 10_000)
      ...
      for i in range(N):
            ...
            # Conversion
            conversion_ratio = conversion_ratio_all[i]
            conversion_price = conversion_price_all[i]
